...
document.addEventListener('DOMContentLoaded', () => {
  const monthDropdown = document.getElementById('lop-month') as HTMLSelectElement;
  const yearDropdown = document.getElementById('lop-year') as HTMLSelectElement;
  ...
  // Set default values
  monthDropdown.value = new Date().getMonth().toString();
  yearDropdown.value = new Date().getFullYear().toString();
...
function generateCalendar(year: number, month: number): void {
  ...
  if (!calendarBody) {
    console.error('lop-calendar tbody not found');
    return;
  }
...
function updateLopData(): void {
  ...
  if (!calendarTitle) {
    console.error('lop-calendar-title not found');
    return;
  }
  calendarTitle.textContent = `${monthDropdown.options[month].text} ${year}`;
              ...
              db.collection('leave_details')
                ...
                .then((querySnapshot: firebase.firestore.QuerySnapshot) => {
                  querySnapshot.forEach((doc: firebase.firestore.QueryDocumentSnapshot) => {
                    ...
                    const dateCell = document.querySelector(`.day-cell[data-date="${leaveDate.getFullYear()}-${leaveDate.getMonth() + 1}-${leaveDate.getDate()}"]`) as HTMLElement;
                  ...
                  const totalPaidDaysElem = document.getElementById('total-paid-days');
                  const fullLopDaysElem = document.getElementById('full-lop-days');
                  const halfLopDaysElem = document.getElementById('half-lop-days');

                  if (!totalPaidDaysElem || !fullLopDaysElem || !halfLopDaysElem) {
                    console.error('One or more lop-container elements not found');
                    return;
                  }

                  totalPaidDaysElem.textContent = `Total Paid Days: ${totalPaidDays}`;
