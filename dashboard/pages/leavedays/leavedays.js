    ...
    function updateCalendarDisplay(leaveData, month, year) {
        ...
        while (date <= daysInMonth) {
            ...
            if (dayOfWeek === 6) {  // Sunday
                ...
                } else {  // Assuming both partial-leave and absent are treated the same way
                    cell.classList.add("full-leave");
                }
            } else if (year == currentYear && month == currentMonth && date > currentDate.getDate()) {
        ...
        const totalFullLeaves = leaveData.filter(leave => leave.remark === "full-leave").length;
        const totalHalfLeaves = leaveData.filter(leave => leave.remark === "partial-leave").length;
